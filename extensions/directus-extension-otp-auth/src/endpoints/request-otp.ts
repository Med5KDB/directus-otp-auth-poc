...
export async function requestOTP(
  ...
  try {
    ...
    if (!/^\+?[1-9]\d{1,14}$/.test(normalizedPhone)) {
      res.status(400).json({
        success: false,
        error: 'Format de numéro de téléphone invalide. Utilisez le format international (+33...)'
      } as OTPResponse);
      return;
    }

    // Vérifier si l'utilisateur existe dans la collection User (utilisateurs finaux)
    ...
    // Invalider tous les codes précédents pour ce téléphone
